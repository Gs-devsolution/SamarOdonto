                                ...
                                <div class="col-sm-12">
                                    <div class="card User-Activity">
                                        ...
                                        <div class="card-block text-center">
                                            ...
                                            {{-- Botão CONFIRMAR: só aparece para admin e status pendente --}}
                                            @if(auth()->user()->type === 'admin' && $appointment->status === 'pending')
                                                <div class="designer m-t-30">
                                                    <a
                                                        href="{{ route('appointments.confirm', $appointment->id) }}"
                                                        class="btn btn-success shadow-2 text-uppercase btn-block"
                                            ...
                                            @endif
                                            {{-- Botão FINALIZAR: só aparece para admin e status confirmado --}}
                                            @if(auth()->user()->type === 'admin' && $appointment->status === 'confirmed')
                                                <div class="designer m-t-30">
                                                    <a
                                                        href="{{ route('appointments.finish', $appointment->id) }}"
                                                        class="btn btn-primary shadow-2 text-uppercase btn-block"
                                                        onclick="return confirm('Finalizar este atendimento? Essa ação é permanente.');"
                                                        style="margin-bottom:12px"
                                                    >
                                                        Finalizar
                                                    </a>
                                            ...
                                            {{-- Botão CANCELAR: aparece para qualquer status que não seja cancelado --}}
