...
class AppointmentService
{
    ...
    public function getPendingAppointments()
    {
        return $this->appointmentModel
            ...
            // ->where('end_date', '>', Carbon::now()->toDateTimeString()) // comente esta linha só para testar
            ->orderBy('start_date')
            ->get();
    }
    ...
    public function getNextAppointments(?int $limit = null)
    {
        ...
        $query = $user->appointments()
            ->whereIn('status', ['pending', 'confirmed'])           // <- só pendentes/confirmadas
    ...
    public function getEndedAppointments(?int $userId = null, ?int $limit = null, bool $isCount = false)
    {
        $query = $this->appointmentModel
            ->where('status', 'finished')           // só finalizados
        ...
        if (!is_null($userId)) {
            ...
            if ($user) {
                ...
                // admin não filtra por usuário (deixa ver todos)
            }
        }
    ...
    public function finish(int $appointmentId)
{
    try {
        $user = auth()->user();

        if ($user->type !== 'admin') {
            return new ServiceResponse(false, 'Usuário não tem permissão para finalizar atendimento');
        }

        $appointment = $this->appointmentModel->find($appointmentId);
        if (is_null($appointment)) {
            return new ServiceResponse(false, 'Agendamento não encontrado');
        }

        // Opcional: só finalizar se estiver confirmado
        if ($appointment->status !== 'confirmed') {
            return new ServiceResponse(false, 'Apenas atendimentos confirmados podem ser finalizados');
        }
        ...
        return new ServiceResponse(false, 'Erro ao finalizar agendamento', null, $th);
    }

    return new ServiceResponse(true, 'Atendimento finalizado com sucesso', $appointment);
}
